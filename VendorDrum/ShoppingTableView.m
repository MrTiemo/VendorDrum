...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section
{
    ...
    if (headModel.headState == 1) {
        ...
        NSString *str = [NSString stringWithFormat:@"%@",[headModel.headPriceDict objectForKey:@"footerTitle"]];
...
//            CGFloat _newprice = allprice * [model.discount floatValue] * 0.1;
            NSString *string = [NSString stringWithFormat:@"%@",model.discount];
            [dict setObject:string forKey:@"headTitle"];
            [dict setObject:[NSString stringWithFormat:@"小计 ¥%.2f",oldprice] forKey:@"footerTitle"];
            [dict setObject:@"" forKey:@"footerMinus"];
            allprice = oldprice;
