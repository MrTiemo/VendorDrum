...
@interface ReturnMoneyViewController ()<UITableViewDelegate,UITableViewDataSource,UITextFieldDelegate>
@property (nonatomic,strong)UITableView *tableView;

@property (nonatomic,strong)UIButton *imageBack; ///<退款原因图片
...
- (void)viewDidLoad {
    ...
    [btnRight addTarget:self action:@selector(tongyiTui) forControlEvents:UIControlEventTouchUpInside];
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ReturnMoneyCell *cell = [tableView cellForRowAtIndexPath:indexPath];
    if (cell==nil) {
        cell = [[ReturnMoneyCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:cellID];
    }
    ...
    if (indexPath.section==0) {
        ...
        cell.labShouHou.text = @"售后方式：退货退款";
        cell.labReason.text = @"售后原因：我不想买了";
        cell.labTuiPrice.text = @"售后金额：¥33.00";
        cell.labShuoMing.text = @"售后说明：坏了，我尼玛，坏了不知道？坏了，我尼玛，坏了不知道？坏了，我尼玛，坏了不知道？";
    
        for (int i=0; i<3; i++) {
            _imageBack = [[UIButton alloc] initWithFrame:CGRectMake(20+50*i+i*10, 120, 50, 50)];
            [_imageBack setBackgroundImage:[UIImage imageNamed:@"tou"] forState:UIControlStateNormal];
            _imageBack.tag = i+1;
            [_imageBack addTarget:self action:@selector(imaBtn:) forControlEvents:UIControlEventTouchUpInside];
            [cell addSubview:_imageBack];
...
-(void)xiugai{
        ...
        ChangeTuiViewController *change = [[ChangeTuiViewController alloc] init];
